  ...
  run(input: RunAgentInput): Observable<BaseEvent> {
    ...
    const agent = new BuiltInAgent({
      ...
      prompt:
        ...
        " For any request that needs current or live information (flight or ticket prices, availability, schedules, news, today's figures), you MUST call browse_web with a sensible URL (for example a Google Flights or Google search URL) and answer from the returned page text. NEVER reply that you cannot browse or that you have no internet access: browsing is available through browse_web. If one site shows a bot check or is blocked, try a different source (Google Flights/aggregators) instead of giving up." +
